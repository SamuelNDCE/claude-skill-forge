...
3. **If a plausible match is found**, state the likely correction and proceed on that basis, flagged clearly rather than silently substituted — e.g. "(reading '[transcribed word]' as '[likely intended term]' — let me know if that's wrong)" — so a genuine unusual request isn't accidentally overridden.
